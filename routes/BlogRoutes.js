...
function handleBlogError(error, res, action) {
  ...
  return res
    .status(500)
    .json({ success: false, message: `Something went wrong while ${action}.` });
}
...
BlogRoute.post("/admin/create",UserMiddleWare,Image_upload.single("image"),async (req, res) => {
    try {
      if (req.user.role !== "admin") {
        return res
          .status(403)
          .json({ success: false, message: "Only admins can create blogs." });
      }
      ...
      if (!title || !title.trim()) {
        return res
          .status(400)
          .json({ success: false, message: "Blog title is required." });
      }
      if (!slug || !slug.trim()) {
        return res
          .status(400)
          .json({ success: false, message: "Blog slug is required." });
      }
      if (!shortDescription || !shortDescription.trim()) {
        return res
          .status(400)
          .json({ success: false, message: "Short description is required." });
      }
      ...
      if (tags) {
        if (Array.isArray(tags)) {
          blogTags = tags.map((tag) => String(tag).trim()).filter(Boolean);
...
BlogRoute.get("/admin", UserMiddleWare, async (req, res) => {
  try {
    if (req.user.role !== "admin") {
      return res
        .status(403)
        .json({ success: false, message: "Only admins can view blogs." });
    }
...
BlogRoute.get("/admin/:id", UserMiddleWare, async (req, res) => {
  try {
    if (req.user.role !== "admin") {
      return res.status(403).json({ success: false, message: "Only admins can view this blog." });
    }
    ...
    if (!blog) {
      return res.status(404).json({ success: false, message: "We couldn't find this blog." });
    }
...
BlogRoute.patch("/admin/:id",UserMiddleWare,Image_upload.single("image"),async (req, res) => {
    try {
      if (req.user.role !== "admin") {
        return res
          .status(403)
          .json({ success: false, message: "Only admins can update blogs." });
      }
      ...
      if (!blog) {
        return res
          .status(404)
          .json({ success: false, message: "We couldn't find this blog." });
      }
      ...
      if (slug !== undefined) {
        if (typeof slug !== "string" || !slug.trim()) {
          return res
            .status(400)
            .json({ success: false, message: "Blog link cannot be empty." });
        }
        const cleanSlug = slug.trim().toLowerCase().replace(/\s+/g, "-");
      ...
      if (req.file) {
        if (blog.imagePublicId) {
          await cleanupDocumentImages(blog, ["imagePublicId"]); // clean up the OLD one first
        }
        const uploadedImage = await uploadToCloudinary(
          req.file.buffer,
          "times-india-travels/blogs",
      ...
      // Same fix as create — clear other featured flags AFTER this blog
      // is successfully saved, and only if it actually ended up featured.
      if (blog.featured) {
        await Blog.updateMany(
          { _id: { $ne: blog._id }, featured: true },
          { $set: { featured: false } },
...
BlogRoute.delete("/admin/:id", UserMiddleWare, async (req, res) => {
  try {
    if (req.user.role !== "admin") {
      return res
        .status(403)
        .json({ success: false, message: "Only admins can delete blogs." });
    }
    ...
    await cleanupDocumentImages(blog, ["imagePublicId"]);
